...
Future<void> generateContractPDFTenant(
    String lessorName,
    String lessorID,
    ...
    String houseRules,
    String? copyOfIDURL,
    String? copyOfStudentURL,
    String principalStamp,
    String? landlordSignatureURL,
    String? signature1URL,
    String? signature2URL,
    String landlordInitialsURL) async {
  final pdf = pw.Document();

  // Fetch the landlord's signature from Firebase
  pw.MemoryImage? image;
  if (landlordSignatureURL != null && landlordSignatureURL.isNotEmpty) {
    final response0 = await http.get(Uri.parse(landlordSignatureURL));
    image = pw.MemoryImage(response0.bodyBytes);
  }

  final response11 = await http.get(Uri.parse(studentSignature));
  final studentImage = pw.MemoryImage(response11.bodyBytes);

  final response12 = await http.get(Uri.parse(waterMarkURL));
  final waterMarkImage = pw.MemoryImage(response12.bodyBytes);

  final response13 = await http.get(Uri.parse(principalStamp));
  final stampImage = pw.MemoryImage(response13.bodyBytes);
  ...
  //tenant documents
  //ID
  pw.MemoryImage? copyOfID;
  if (copyOfIDURL != null && copyOfIDURL.isNotEmpty) {
    final response1 = await http.get(Uri.parse(copyOfIDURL));
    copyOfID = pw.MemoryImage(response1.bodyBytes);
  }
  //Student Card
  pw.MemoryImage? copyOfStudentCard;
  if (copyOfStudentURL != null && copyOfStudentURL.isNotEmpty) {
    final response1 = await http.get(Uri.parse(copyOfStudentURL));
    copyOfStudentCard = pw.MemoryImage(response1.bodyBytes);
  }
  ...
  pdf.addPage(pw.MultiPage(
    ...
    return [
      ...
      pw.Center(
        child: pw.Column(
          children: [
            ...
            pw.Text('$lessorName \n (HENCEFORWARD THE LESSOR)',
                textAlign: pw.TextAlign.center,
                style:
                    pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold)),
            pw.Text('AND \n $lesseeName \n (HENCEFORWARD THE LESSEE)',
                textAlign: pw.TextAlign.center,
                style:
                    pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold)),
            pw.Text('Property: $propertyName, Unit: $roomName',
                textAlign: pw.TextAlign.center,
                style:
                    pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold)),
      ...
      pw.Text('Name: $lessorName', style: pw.TextStyle(fontSize: 10)),
      pw.Text('Company/Lessor ID: $lessorID',
          style: pw.TextStyle(fontSize: 10)),
      pw.Text('Tel Number: $lessorTel', style: pw.TextStyle(fontSize: 10)),
      pw.Text('Email: $lessorEmail', style: pw.TextStyle(fontSize: 10)),
      pw.Text(
      ...
      pw.Text('Lessee Information',
          style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)),
      pw.Text('Name: ${lesseeName ?? 'lessee name'.toUpperCase()}',
          style: pw.TextStyle(fontSize: 10)),
      pw.Text('Lessee ID: ${lesseeID ?? 'lesseeID'}',
          style: pw.TextStyle(fontSize: 10)),
      pw.Text('Tel Number: ${lesseeTel ?? 'lessee telephone'}',
          style: pw.TextStyle(fontSize: 10)),
      pw.Text('Email: ${lesseeEmail ?? 'lessee email'}',
          style: pw.TextStyle(fontSize: 10)),
      ...
      pw.Text('Next of Kin Information',
          style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)),
      pw.Text('Name: $nextOfKinName', style: pw.TextStyle(fontSize: 10)),
      pw.Text('ID: $nextOfKinID', style: pw.TextStyle(fontSize: 10)),
      pw.Text('Tel Number: $nextOfKinTel', style: pw.TextStyle(fontSize: 10)),
      pw.Text('Email: $nextOfKinEmail', style: pw.TextStyle(fontSize: 10)),
      pw.Text(
      ...
      // PRTR information
      pw.Text('Person responsible to pay rent Information',
          style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)),
      pw.Text('Name: $sponsorName', style: pw.TextStyle(fontSize: 10)),
      pw.Text('ID: $sponsorID', style: pw.TextStyle(fontSize: 10)),
      pw.Text('Tel Number: $sponsorTel', style: pw.TextStyle(fontSize: 10)),
      pw.Text('Email: $sponsorEmail', style: pw.TextStyle(fontSize: 10)),
      pw.Text(
      ...
      //the rules regarding rent and all of that stuff

      //
      // SHARING ROOMS CONDITION
      //
      if (roomType.toUpperCase() == 'SHARING')
        pw.Center(
          child: pw.Column(children: [
            pw.Text('Sharing Room',
                style:
            ...
            pw.Text(
                'Rent: R ' +
                    ...
                    totalRent.toString(),
                style: pw.TextStyle(fontSize: 10)),
            pw.Text('Refundable Fee: R$refundableFee',
                style: pw.TextStyle(fontSize: 10)),
            pw.Text('Admin Fee: R$adminFee', style: pw.TextStyle(fontSize: 10)),
            pw.Text('Deposit: R$deposit', style: pw.TextStyle(fontSize: 10)),
            pw.Text('Key Deposit: R$keyDeposit',
                style: pw.TextStyle(fontSize: 10)),
            pw.Text(
            ...
            pw.Bullet(
                ...
                style: pw.TextStyle(fontSize: 10)),
            pw.Text(
                'NB! People staying in sharing rooma are responsible to buy their own electricity for the room, provided there is a prepaid meter in the room.'),
            pw.SizedBox(height: 20),
          ]),
        ),
      //
        ...
        pw.Column(children: [
          ...
          pw.Text(
              'Rent: R ' +
                  ...
                  totalRent.toString(),
              style: pw.TextStyle(fontSize: 10)),
          pw.Text('Refundable Fee: R$refundableFee (REFUNDABLE)',
              style: pw.TextStyle(fontSize: 10)),
          pw.Text('Admin Fee: R$adminFee (NON-REFUNDABLE)',
              style: pw.TextStyle(fontSize: 10)),
          pw.Text('Deposit: R$deposit', style: pw.TextStyle(fontSize: 10)),
          pw.Text('Key Deposite: R$keyDeposit (NON-REFUNDABLE)',
              style: pw.TextStyle(fontSize: 10)),
          pw.Text(
              'R1000 Electricity fee if move in before 20 January ' +
                  currentDate.year.toString(),
          ...
          pw.SizedBox(height: 10),
          pw.Text(
              'The tenant will be eligible to receive their refund back (only if full amount was paid) if the following criteria are met during the course of the year:',
              style: pw.TextStyle(fontSize: 10)),
          pw.SizedBox(height: 10),
          pw.Bullet(
              text:
          ...
          pw.Bullet(
              text:
                  'Tenant\'s keys are handed in at the office before leaving Bloemfontein at the end of the year. Keys will be checked to see if the correct keys were handed back and if all  the keys are there that were originally given to the tenant.',
              style: pw.TextStyle(fontSize: 10)),
          pw.Bullet(
              text:
                  'No damage has been caused to the room/bed/desk and chair by the tenants.',
          ...
          pw.Bullet(
              text:
                  'The room has been thoroughly cleaned by the tenant and all the rubbish, unwanted books and food is thrown away.',
              style: pw.TextStyle(fontSize: 10)),
          pw.SizedBox(height: 10),
          pw.Text(
              'NB! People staying in single rooms are responsible to buy their own electricity for the room they occupy, provided that the room has a prepaid meter.',
              style: pw.TextStyle(fontSize: 10)),
          pw.SizedBox(height: 20),
        ]),
      //
      //Bachlor Rooms
      if (roomType.toUpperCase() == 'BACHELOR')
        pw.Center(
          child: pw.Column(children: [
            pw.Text('Bachlor Room',
                style:
                    pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold)),
            pw.SizedBox(height: 10),
            pw.Text(
            ...
            pw.SizedBox(height: 10),
            pw.Text(
                'NB! People staying in bachlor rooms are responsible to buy their own electricity for the room they occupy, provided that the room has a prepaid meter.',
                style: pw.TextStyle(fontSize: 10)),
            pw.SizedBox(height: 20),
          ]),
        ),
      ...
      if (roomType.toUpperCase() == 'FLAT')
        pw.Column(children: [
          ...
          pw.SizedBox(height: 10),
          pw.Text(
              'NB! People staying in flats are responsible to buy their own electricity for the room they occupy, provided that the room has a prepaid meter.',
              style: pw.TextStyle(fontSize: 10)),
          pw.SizedBox(height: 20),
        ]),
      //
      //Townhouse
      if (roomType.toUpperCase() == 'TOWN HOUSE')
        pw.Center(
          child: pw.Column(children: [
            pw.Text('Town House',
                style:
      ...
      //
      //Commercial Property
      if (roomType.toUpperCase() == 'COMMERCIAL')
        pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.center, children: [
          pw.Text('Commercial Property',
              style:
                  pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold)),
          pw.SizedBox(height: 10),
          pw.Text(
      ...
      //
      //Custom Room Type
      if (roomType.toUpperCase() == 'OTHER')
        pw.Center(
          child: pw.Column(
            crossAxisAlignment: pw.CrossAxisAlignment.center,
            children: [
              pw.Text('$customRoomType',
                  style: pw.TextStyle(
      ...
      pw.Text('Bank details:',
          style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)),
      pw.Text('First National Bank',
          style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)),
      pw.Text('Account Number: 63039110810',
          style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)),
      pw.Text('Trust Account',
          style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)),
      pw.Text('Branch Code: 250051',
          style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)),
      pw.Text('Branch Name: Langenhoven park',
          style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)),
      pw.Text('REF: $propertyName/$studentNumber',
          style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)),
      pw.SizedBox(height: 10),
      //pw.NewPage(),
      ...
      pw.Bullet(text: '$topTen1', style: pw.TextStyle(fontSize: 10)),
      pw.Bullet(text: '$topTen2', style: pw.TextStyle(fontSize: 10)),
      pw.Bullet(text: '$topTen3', style: pw.TextStyle(fontSize: 10)),
      pw.Bullet(text: '$topTen4', style: pw.TextStyle(fontSize: 10)),
      pw.Bullet(text: '$topTen5', style: pw.TextStyle(fontSize: 10)),
      pw.Bullet(text: '$topTen6', style: pw.TextStyle(fontSize: 10)),
      pw.Bullet(text: '$topTen7', style: pw.TextStyle(fontSize: 10)),
      pw.Bullet(text: '$topTen8', style: pw.TextStyle(fontSize: 10)),
      pw.Bullet(text: '$topTen9', style: pw.TextStyle(fontSize: 10)),
      pw.Bullet(text: '$topTen10', style: pw.TextStyle(fontSize: 10)),
      pw.SizedBox(height: 20),
      ...
      pw.Padding(
        ...
        child: pw
          ...
          pw.Text(
              '1. The lessee occupies the following part of the property: Property: $propertyName, Unit: $roomName.',
              style: pw.TextStyle(fontSize: 10)),
          pw.SizedBox(height: 10),
          pw.Text(
              '2. This rental contract will start on ' +
                  ...
                  ' .All items must be removed from the rooms including the fridge 7 days after classes and exams end at the end of 2025. We have the right to remove everything from the date your contract ends, if you have not renewed your contract in writing.',
              style:
                  pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)),
          pw.SizedBox(height: 10),
          pw.Text(
              '3. Should the lessee wish to vacate the property within this contract period, he/she is responsible for finding a new tenant agreed upon with the agent. The tenant will sign a new agreement for the room.',
              style:
                  pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)),
          pw.SizedBox(height: 10),
          pw.Text(
          ...
          pw.SizedBox(height: 10),
          pw.Text(
              '5. $lessorName reserves the right to move cash student to another property if needed.',
              style: pw.TextStyle(fontSize: 10)),
          pw.SizedBox(height: 10),
          pw.Text(
              '6. In the event of a student wanting to move to another room on the same property or to a room in another property the student needs to pay a relocation fee of R1500 before keys will be released for the new room.',
              style: pw.TextStyle(fontSize: 10)),
          pw.SizedBox(height: 10),
          pw.Text(
              '7. No termination letters will be released to a student within 30 days prior to the closing date of Private Accommodation applications.',
              style:
                  pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)),
          pw.SizedBox(height: 10),
          ...
          pw.Text('9.1 Geyser Rules: $geyserRules',
              style:
                  pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)),
          pw.SizedBox(height: 10),
          pw.Text(
              '10. No room keys will be released without proof of payment of all amounts.',
              style: pw.TextStyle(fontSize: 10)),
          pw.SizedBox(height: 10),
          pw.Text(
              '10.1 Tenants will always accept communal and separate responsibility for any damage caused to the part of the leased property occupied by them, which will include all appliances. Damages will be deducted from student credit/deposit.',
              style: pw.TextStyle(fontSize: 10)),
          pw.SizedBox(height: 10),
          pw.Text(
              '11. Sharing rooms that are not occupied by 2 tenants will be seen as single rooms and will be billed as single rooms. The following options are available at $lessorName discretion.',
              style: pw.TextStyle(fontSize: 10)),
          pw.SizedBox(height: 10),
          pw.Text(
            ...
            children: [
              ...
              pw.Container(
                ...
                child: pw.FittedBox(
                  ...
                  child: pw.Image(studentImage as pw.ImageProvider),
                ),
              ),
            ],
          ),
          ...
          pw.Center(
              child: pw.Column(children: [
            pw.Text(
                'PLEASE NOTE: ALL REFUNDS WILL BE PROCESSED BY LATEST END OF JANUARY 2026',
                style:
                    pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold))
          ])),
          ...
          pw.Text(
              '13. If the lessee finds with occupation of the property that the property or any equipment or contents thereof, ' +
                  ...
                  'and appropriate condition to the lessor at the termination of the rental term, acceptable and reasonable wear ' +
          ...
          pw.SizedBox(height: 10),
          pw.Text(
              '14. Every tenant is responsible for his own door. If for any reason the door or a window is broken, kicked or damaged in any ' +
                  'way the tenant will be responsible for getting them fixed to the original state they were found in when lease was ' +
                  'signed. The tenant has 7 days from moving in to give us notice of damaged door. Notice can be given on RedRabbit',
              style: pw.TextStyle(fontSize: 10)),
          pw.SizedBox(height: 10),
          pw.Text(
          ...
          pw.Text(
              '25. If the rent, as noted, or any other amount or amounts payable to the lessor by the lessee is not paid ' +
                  ...
                  'contract by the lessee, including legal costs. Any permission or lenience from the lessor ' +
                  'regarding to the breaking of any stipulations of this contract by the lessee will not be taken into ' +
          ...
          pw.Bullet(
              text: 'NO 2 PLATE STOVES OR ANY HEATERS ARE ALLOWED IN ANY ROOM. It will be confiscated ' +
                  'and will only be returned when the student moves out. Fan heaters & electric blankets will only be allowed if the ' +
          ...
          pw.SizedBox(height: 10),
          pw.Text(
              '30. The lessee must occupy the house with the consideration of the rights of the other tenants and neighbours. ' +
                  'The wi-fi may temporarily be switched off should a single tenant or more tenants not cooperate and adhere ' +
                  'to the contract or House rules or in the case where there are behavioural challenges that needs to be ' +
          ...
          pw.Text(
              ...
              style: pw.TextStyle(fontSize: 10)),
          pw.SizedBox(height: 10),
          pw.Text('36. The following is agreed upon regarding to guests:',
              style: pw.TextStyle(fontSize: 10)),
          pw.SizedBox(height: 10),
          pw.Bullet(
      ...
      pw.SizedBox(height: 10),
      pw.Text('Signed Electronically on 01 February 2025',
          style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)),
      pw.SizedBox(height: 10),
      pw.Text(
        ...
        pw.Row(
          ...
          children: [
            ...
            pw.Container(
              ...
              child: pw.FittedBox(
                fit: pw.BoxFit.contain,
                child: pw.Image(signature1 as pw.ImageProvider),
              ),
            ),
          ],
        ),
        ...
        pw.Row(
          ...
          children: [
            ...
            pw.Container(
              ...
              child: pw.FittedBox(
                fit: pw.BoxFit.contain,
                child: pw.Image(signature2 as pw.ImageProvider),
              ),
            ),
          ],
        ),
      ...
      pw.Text('HOUSE RULES AND FINES APPLICABLE:',
          style: pw.TextStyle(fontSize: 10)),
      pw.SizedBox(height: 5),
      pw.Paragraph(text: '$houseRules', style: pw.TextStyle(fontSize: 10)),
      pw.SizedBox(height: 10),
      pw.Text(
      ...
      pw.Text('Annexure C',
          style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)),
      pw.SizedBox(height: 20),
      pw.Text('AMMENDMENT TO HOUSE RULES 2025',
          style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)),
      pw.SizedBox(height: 10),
      ...
      pw.Bullet(
          text: 'Remove or damage fitted sheet that is supplied for your mattress. Wash the fitted sheet regularly. If you ' +
              ...
              'should you have damaged or neglected it to the point that it cannot be used.',
          style: pw.TextStyle(fontSize: 10)),
      pw.Bullet(
          text:
      ...
      pw.Bullet(
          ...
          style: pw.TextStyle(fontSize: 10)),
      pw.Bullet(
          text: 'Leave your personal cutlery and pots unwashed after use. Please clean immediately after you cooked because ' +
              'dirty dishes and dirty counter space creates the perfect environment for pests and rodents. It is not our ' +
      ...
      pw.Bullet(
          ...
          style: pw.TextStyle(fontSize: 10)),
      pw.Bullet(
          text: 'Stoves, hobs, microwave and kettles: These are a high energy (electricity) consuming ' +
              'appliances and should be switched off as soon as possible after it has been used. There is a limit to the ' +
      ...
      pw.Bullet(
          text:
              'Create mould in bathrooms, open window after every use. Leave the shower door open when not in the bathroom, this helps to air the shower and mould does not grow.',
          style: pw.TextStyle(fontSize: 10)),
      pw.Bullet(
          text: 'Leave a mess in the bathrooms.',
          style: pw.TextStyle(fontSize: 10)),
      pw.Bullet(
          text: 'Leave the toilet unflushed or dirty.',
          style: pw.TextStyle(fontSize: 10)),
      pw.Bullet(
          text: 'Use anything but toilet paper. Should the drain system be blocked due to any foreign items for example ' +
              'newspapers, sanitary products, and hair. The cost will be recovered from the students using the bathroom. ',
          style: pw.TextStyle(fontSize: 10)),
      pw.Bullet(
      ...
      pw.Bullet(
          text:
              'Waste water. Close taps that are not in use as there is a limit to the amount of water bought for the house per week.',
          style: pw.TextStyle(fontSize: 10)),
      pw.SizedBox(height: 10),
      //WI-FI
      pw.Text('Wi-Fi',
      ...
      pw.Bullet(
          text:
              'Abuse washing machine. Remove any change, tissues, or any other items that can damage the washing machine before your wash. Clothing only to be thrown in ' +
                  'the drum and not where the powder or detergent must go in',
          style: pw.TextStyle(fontSize: 10)),
      pw.Bullet(
          text:
      ...
      pw.SizedBox(height: 10),
      pw.Bullet(
          text: 'Treat your keys as if it is something that can be replaced without any consequences or second thought. ' +
      ...
      pw.SizedBox(height: 10),
      pw.Text(
          'The tenant hereby consents that, and authorizes the landlord or agent to, always:',
          style: pw.TextStyle(fontSize: 10)),
      pw.SizedBox(height: 10),
      pw.Bullet(
          text: 'Contact, request, share and obtain information from any credit provider (or potential credit provider) or registered ' +
              'credit bureau relevant to an assessment of the behaviour, profile, payment patterns, indebtedness, whereabouts ' +
              'and credit worthiness of the tenant.',
          style: pw.TextStyle(fontSize: 10)),
      pw.Bullet(
          text: 'Furnish information concerning the behaviour, profile, payment patterns, indebtedness, whereabouts and ' +
      ...
      pw.Text(
          'This lease agreement was presented to me in plain language and that I understand the contents thereof. I was ' +
              ...
              'and no unfair tactics or any other similar conduct regarding to the negotiation, conclusion, execution or ' +
              ...
              'this agreement without viewing the property beforehand, and therefore waive any right to cancel the contract or ' +
      ...
      pw.SizedBox(height: 20),
      pw.Image(stampImage as pw.ImageProvider, width: 150),
      pw.NewPage(),
      if (signature1 != null)
        pw.Row(
          mainAxisAlignment: pw.MainAxisAlignment.start,
          ...
          children: [
            ...
            pw.Container(
              ...
              child: pw.FittedBox(
                fit: pw.BoxFit.contain,
                child: pw.Image(copyOfID as pw.ImageProvider),
              ),
            ),
          ],
        ),
        ...
        pw.Row(
          ...
          children: [
            ...
            pw.Container(
              ...
              child: pw.FittedBox(
                fit: pw.BoxFit.contain,
                child: pw.Image(copyOfStudentCard as pw.ImageProvider),
              ),
            ),
          ],
        ),
    ...
    return pw.Container(
      ...
      child: pw.Opacity(
        ...
        child: pw.Row(mainAxisAlignment: pw.MainAxisAlignment.end, children: [
          pw.Image(waterMarkImage as pw.ImageProvider, height: 20, width: 20),
          pw.Text(' Made in Letify', style: pw.TextStyle(fontSize: 10))
      ...
      child: pw.Row(
        ...
        children: [
          ...
          pw.Column(
            ...
            children: [
              pw.Image(studentImage, width: 30),
              if (signature1 != null) ...[
                pw.SizedBox(height: 4),
                pw.Image(signature1, height: 70, width: 70),
              ],
            ],
          ),
          ...
          pw.Column(
            mainAxisSize: pw.MainAxisSize.min,
            children: [
              pw.Image(image as pw.ImageProvider, width: 30),
              if (signature2 != null) ...[
                pw.SizedBox(height: 4),
                pw.Image(signature2, height: 70, width: 70),
              ],
            ],
          ),
        ],
